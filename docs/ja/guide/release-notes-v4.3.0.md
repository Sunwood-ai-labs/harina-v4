...
- Gemini キー回転時の再試行を遅延付きで補完し、常時稼働の bot / watcher フローの耐障害性を向上
...
本番フローとテスト/検証フローの Gemini 利用モデルを分離し、Drive/Discord の実運用を止めずに検証フローを回しやすくしました。加えて、常時稼働の bot / watcher フローではキー回転枠の枯渇時に遅延付き再試行を行い、一時的な失敗時の回復性を上げています。
...
Drive watcher の receipt 埋め込みに、利用モデルと推定コスト情報を表示するようになり、運用時の原因調査やコスト観点の確認を行いやすくなります。
...
- [Harina v4.3.0 解説](/ja/guide/whats-new-v4.3.0)
