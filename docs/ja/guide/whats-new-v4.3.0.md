...
あわせて、キー回転が尽きたときの待機付き再試行も入り、Gemini 呼び出しまわりの回復性が上がっています。さらに Drive Receipt embed には使用モデル名と推定 API cost が出るため、品質とコストを同じ文脈で確認しやすくなりました。
...
- Discord embed と watcher ログから、モデルやコストの確認がしやすくなります。
